...
**Settlement on EVM-compatible chains.** All on-chain activity — confidential transfers, mints, redemptions, balance state — settles on EVM-compatible chains (e.g. **Ethereum**, **Polygon**, **Base**, **Arc**, **Plasma**, **Tempo**, among others). Chain target and RPC endpoint are configurable per deployment. See [supported networks](/reference/supported-networks) for the canonical list and roadmap.
